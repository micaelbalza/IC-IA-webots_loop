...
function PSO_ControlProgram(destination_path, population_size, max_generations, self_adjustment_weight, social_adjustment_weight, robot_radius, Alpha)
    ...
    self_adjustment_weight = str2double(self_adjustment_weight);
    social_adjustment_weight = str2double(social_adjustment_weight);

    disp(population_size)
    disp(max_generations)
    disp(self_adjustment_weight)
    disp(social_adjustment_weight)

    % Opitions --> PSO
    options.SwarmSize = population_size;
    options.MaxIterations = max_generations;
    options.SelfAdjustmentWeight = self_adjustment_weight;  
    options.SocialAdjustmentWeight = social_adjustment_weight; 
